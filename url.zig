...
pub const SearchParams = struct {
    ...
    pub fn init(alloc: std.mem.Allocator) SearchParams {
        return .{
            .allocator = alloc,
            .inner = std.MultiArrayList(KV){},
        };
    ...
    pub fn initFromString(alloc: std.mem.Allocator, input: string) !SearchParams {
        var uv = SearchParams.init(alloc);
    ...
    pub fn set(self: *SearchParams, key: string, value: string) !void {
    ...
    pub fn append(self: *SearchParams, key: string, value: string) !void {
        try self.inner.append(self.allocator, .{ .key = key, .value = value });
    }

    pub fn get(self: *const SearchParams, key: string) ?string {
        const keys = self.inner.items(.key);
    ...
    pub fn getAll(self: *const SearchParams, key: string) !?[]const string {
        const keys = self.inner.items(.key);
        const values = self.inner.items(.value);
    ...
    pub fn take(self: *SearchParams, key: string) ?string {
        const keys = self.inner.items(.key);
        const idx = extras.indexOfSlice(u8, keys, key) orelse return null;
        const values = self.inner.items(.value);
    ...
    pub fn delete(self: *SearchParams, key: string, value: ?string) void {
        var offset: usize = 0;
        while (true) {
            const keys = self.inner.items(.key);
            const values = self.inner.items(.value);
    ...
    pub fn has(self: *const SearchParams, key: string, value: ?string) bool {
        const keys = self.inner.items(.key);
        if (value == null) {
    ...
    fn findLastSet(self: std.bit_set.DynamicBitSetUnmanaged) ?usize {
...
pub fn is_component_percent_char(c: u8) bool {
    if (c >= '$' and c <= '&') return true;
    if (c == '+') return true;
    if (c == ',') return true;
...
pub fn is_formurlencoded_percent_char(c: u8) bool {
    if (c == '!') return true;
    if (c >= '\'' and c <= ')') return true;
    if (c == '~') return true;
